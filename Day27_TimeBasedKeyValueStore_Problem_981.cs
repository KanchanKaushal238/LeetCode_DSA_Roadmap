...
public class TimeMap {
    Dictionary<string, List<TimeDetail>> data;
    public TimeMap()
    {
        data = new Dictionary<string, List<TimeDetail>>();
    }
    ...
    public void Set(string key, string value, int timestamp)
    {
        if(!data.ContainsKey(key))
        {
            ...
            list.Add(new TimeDetail(value, timestamp));

            data.Add(key, list);
        }
        else
        {
            var dataDict = data[key];
            dataDict.Add(new TimeDetail(value, timestamp));
        }
    }
    ...
    public string Get(string key, int timestamp)
    {
        ...
        if (data.ContainsKey(key))
        {
            ...
            if(string.IsNullOrWhiteSpace(result) && right >= 0 && list[right].timestamp <= timestamp)
            {
                result = list[right].value;
            }
            else if (string.IsNullOrWhiteSpace(result) && left < list.Count && list[left].timestamp <= timestamp)
            {
                result = list[left].value;
            }
        }
